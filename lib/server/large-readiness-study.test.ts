...
beforeEach(() => {
  vi.clearAllMocks()
  rpc.mockImplementation(async (_client: unknown, name: string) => name === 'claim_study_run_batch' ? [target] : true)
...
describe('large research runner', () => {
  ...
  it('removes all raw identity and page text from the observation', () => {
    ...
    expect(row.metrics).toMatchObject({ source: 'study', scanner_version: 2, http_status: 200, has_visible_price: true })
  ...
  it('bounds intra-invocation concurrency at three', async () => {
    rpc.mockImplementation(async (_client: unknown, name: string) => name === 'claim_study_run_batch' ? Array.from({ length: 6 }, (_, i) => ({ ...target, id: String(i) })) : true)
