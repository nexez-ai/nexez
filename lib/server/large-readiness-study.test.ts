...
const result: SiteSignalsResult = {
  ...
  researchQuality: { protocolVersion: 6, failure: null, diagnostics: { visibleChars: 120, replacementChars: 0, htmlBytes: 200, titleChars: 12 } },
}
...
beforeEach(() => {
  vi.clearAllMocks()
  rpc.mockImplementation(async (_client: unknown, name: string) => name === 'study_run_status' ? { researchProtocolVersion: 6 } : name === 'claim_study_run_batch' ? [target] : true)
...
describe('large research runner', () => {
  it('requires fresh protocol-6 numeric evidence and never copies a legacy score', () => {
    expect(buildResearchObservation({ ...result, researchQuality: { protocolVersion: 6, failure: null } }, 'test-cohort', 'retail')).toBeNull()
    expect(buildResearchObservation(result, 'test-cohort', 'retail')?.metrics).toMatchObject({
      research_protocol_version: 6, research_visible_chars: 120, research_replacement_chars: 0,
  ...
  it('stops a sealed study before network work when its hash configuration changes', async () => {
    rpc.mockResolvedValue({ researchProtocolVersion: 6, hashIdentityFingerprint: '0'.repeat(64) })
  ...
  it('continues with the same sealed identity without changing scoring', async () => {
    rpc.mockImplementation(async (_client: unknown, name: string) => name === 'study_run_status'
      ? { researchProtocolVersion: 6, hashIdentityFingerprint: researchHashIdentityFingerprint() }
  ...
  it('removes all raw identity and page text from the observation', () => {
    ...
    expect(row.metrics).toMatchObject({ source: 'study', scanner_version: 2, research_protocol_version: 6, http_status: 200, has_visible_price: true })
  ...
  it.each(['non_html','insufficient_content','challenge_page','parked_domain','unavailable_page','excluded_destination'] as const)('records %s without scoring or persisting text', async (failure) => {
    gather.mockResolvedValue({ ...result, researchQuality: { protocolVersion: 6, failure } })
  ...
  it.each([undefined, 1, 2, 3, 4, 5, 7])('does not claim a cohort with incompatible protocol %s', async (researchProtocolVersion) => {
  ...
  it('uses research capacity and fenced atomic completion', async () => {
    ...
    expect(gather).toHaveBeenCalledWith('https://example.com', expect.objectContaining({ researchProtocolVersion: 6 }))
  ...
  it('does not finalize another invocation on dispatch replay', async () => {
    rpc.mockImplementation(async (_client: unknown, name: string) => name === 'study_run_status' ? { researchProtocolVersion: 6 } : [])
  ...
  it('leaves unpersisted targets for lease recovery', async () => {
    rpc.mockImplementation(async (_client: unknown, name: string) => {
      if (name === 'study_run_status') return { researchProtocolVersion: 6 }
  ...
  it('bounds intra-invocation concurrency at three', async () => {
    rpc.mockImplementation(async (_client: unknown, name: string) => name === 'study_run_status' ? { researchProtocolVersion: 6 } : name === 'claim_study_run_batch' ? Array.from({ length: 6 }, (_, i) => ({ ...target, id: String(i) })) : true)
