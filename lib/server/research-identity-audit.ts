...
export async function readResearchIdentityAudit(cohort: string) {
  ...
  if (![4, 5, 6, 7].includes(Number(status.researchProtocolVersion)) || !['pilot_review', 'paused', 'exhausted', 'completed'].includes(String(status.state))) {
    throw new Error('Identity review requires a stopped protocol-4, protocol-5, protocol-6 or protocol-7 run')
  }
