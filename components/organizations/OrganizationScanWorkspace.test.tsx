...
describe('scan workspace interactions', () => {
  it('requires valid targets and attestation, preserves the request key on a failed submission', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Scan 1 website' })).toBeDisabled()
    fireEvent.click(screen.getByRole('checkbox'))
    fireEvent.click(screen.getByRole('button', { name: 'Scan 1 website' }))
    await screen.findByText('Please retry.')
    await waitFor(() => expect(screen.getByRole('button', { name: 'Scan 1 website' })).toBeEnabled())
    fireEvent.click(screen.getByRole('button', { name: 'Scan 1 website' }))
